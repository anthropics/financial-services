...
def _json_object_at(text: str, start: int) -> str | None:
    """Return the JSON object substring starting at `start`, or None.

    Walks the text honoring brace depth and JSON string escaping so a nested
    `}` inside `payload.event` (or inside any quoted string) doesn't terminate
    the scan early. Replaces the previous non-greedy regex which truncated at
...
def _handoff_candidates(text: str):
    """Yield JSON object substrings that contain a `"type":"handoff_request"`.

    For each `"type":"handoff_request"` marker we walk backwards to find the
    enclosing `{`, then forward to find its matching `}`. This handles cases
    where the marker isn't inside the outermost object on the line.
    """
