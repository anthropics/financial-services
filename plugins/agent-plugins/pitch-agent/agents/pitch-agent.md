...
tools: Read, Write, Edit, mcp__capiq__*, mcp__ibkr__*
---
...
3. **Pull data.** Start with the IBKR MCP when reachable (`ibkr_quote`,
   `ibkr_fundamentals`, `ibkr_historical` for the target and the
   comp set's live trading layer). Use the CapIQ MCP for trading
   multiples, precedent transaction data, and the target's latest
   filings. Load full filings — do not summarize from snippets.
...
- **Cite every number.** If a multiple or precedent can't be sourced from CapIQ, IBKR, or a filing, flag it as `[UNSOURCED]` rather than estimating.
